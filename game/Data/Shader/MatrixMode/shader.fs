...
uniform vec2 size;
uniform vec2 pixel;//=vec2(100,50);
...
void main () {
	...
	coord.y=-coord.y;
	gl_FragColor =( texture(char,coord))*vec4(0.,1.0,0.,1.)*c+texture(texture,tc)/2;//vec4((coord+getChar(c)),0,1);//texture (char,(tc/16+getChar(c))*pixel);//vec4(0.,0.,0.,1.)+frag;
	gl_FragColor.a=1.;
}